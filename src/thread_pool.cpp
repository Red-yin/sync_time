...
threadpool* ThreadPool::threadpool_init(int thread_num, int queue_max_num)
{
...
{
    return threadpool_add_job(pool, callback_function, arg);
}

int ThreadPool::threadpool_add_job(threadpool* pool, void* (*callback_function)(void *arg), void *arg)
{
//    assert(pool != NULL);
